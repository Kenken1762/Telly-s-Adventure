...
public class MainActivity extends AppCompatActivity
{
    ...
    public void mainMenuStart(View view)
    {
       try{thread.interrupt();}catch(Exception e){}
        ...
        Intent mainMenuIntent = new Intent(MainActivity.this, StartMenu.class);
    ...
    public void levelTwoStart(View view)
    {
        ...
        try{runAnimation.stop();}catch(Exception e){}
        try{jumpAnimation.stop();}catch(Exception e){}
        Intent levelTwoIntent = new Intent(MainActivity.this, MainActivity2.class);
...
class Platforms extends AppCompatActivity
{
    ImageView id;

    public Platforms(ImageView iD)
    {
        id = iD;
    }

    public ImageView getId() {return id;}
}

class Ground //extends MainActivity
{
    public boolean onGround(boolean verticalPressed, float heroJumpY,  float heroJumpX, ImageView heroJump, FrameLayout frame, ArrayList<Platforms> platforms)
    {
        if(heroJumpY == frame.getHeight() - heroJump.getHeight() - 81)
            return true;

        for(int x = 0; x < platforms.size(); x++)
            if((heroJumpY >= platforms.get(x).getId().getY() - heroJump.getHeight() - 1  && heroJumpY <= platforms.get(x).getId().getY()) &&
                    ((heroJumpX >= platforms.get(x).getId().getX() - heroJump.getWidth()) && (heroJumpX <= platforms.get(x).getId().getX() + platforms.get(x).getId().getWidth()))
                    && verticalPressed)
                return true;
